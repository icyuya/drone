...
void init_copter(void)
{
  ...
  // USBSerial.print("StampFly MAC Address: ");
...
void led_drive(void)
{
  ...
  else if (Mode == FLIGHT_MODE)
  {
    ...
    if (Under_voltage_flag < UNDER_VOLTAGE_COUNT)
      onboard_led(Led_color, 1);
    else
      onboard_led(POWEROFFCOLOR, 1);
  }
  else if (Mode == PARKING_MODE)
  {
    if (LedBlinkCounter == 0)
    { //<10
      if (Led_color2 & 0x000001)
        Led_color2 = (Led_color2 >> 1) | 0x800000;
      else
        Led_color2 = Led_color2 >> 1;
      if (Under_voltage_flag < UNDER_VOLTAGE_COUNT)
        onboard_led(Led_color2, 1); // GREEN
      else
        onboard_led(POWEROFFCOLOR, 1);
      LedBlinkCounter++;
    }
